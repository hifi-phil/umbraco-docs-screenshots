...
- **`0`** — proceed to Step 3. In targeted mode this includes the case where the guard tripped but is
  ...
  them.
- **`1`** — **default mode and explicit Slack mode.** The guard tripped: report the already-open
  PR(s) and **end the run** — nothing to do until a reviewer merges or closes them. If a message
...
targeted mode and any Slack-sourced run the filename is `$(basename "$IMG")` — this grep is how you
...
capture. Targeted mode may ask whether to continue anyway (per the autonomy note above); the
discovery-fallback phase just ends the run (a future run tries again); a Slack-sourced run replies
`❌ Errored: image isn't referenced by any article — nothing to capture against` and moves on.
